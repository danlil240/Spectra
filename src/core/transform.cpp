...
Vec2 data_to_ndc(float data_x, float data_y,
    ...
    return {
        2.0f * (data_x - x_min) / range_x - 1.0f,
        2.0f * (data_y - y_min) / range_y - 1.0f
    };
}

Vec2 ndc_to_screen(float ndc_x, float ndc_y, const Rect& viewport) {
    return {
        viewport.x + (ndc_x + 1.0f) * 0.5f * viewport.w,
        viewport.y + (ndc_y + 1.0f) * 0.5f * viewport.h
    };
}

Vec2 data_to_screen(float data_x, float data_y,
                    float x_min, float x_max,
                    float y_min, float y_max,
                    const Rect& viewport) {

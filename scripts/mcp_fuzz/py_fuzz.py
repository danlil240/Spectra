...
def fuzz_loop(steps: int, log_path: Path) -> int:
    ...
    with log_path.open("a", encoding="utf-8") as log_file:
        for step in range(1, steps + 1):
            ...
            if not alive():
                ...
                return step
            pump_count = structured(response).get("pump_frames", 2) or 2
...
def command_exhaustion(tag: str) -> None:
...
def main() -> int:
    ...
    completed = fuzz_loop(FUZZ_STEPS, log_path)
    scan_stderr(stderr)

    if alive():
        command_exhaustion(tag)
    if alive():
    ...
    print(f"DONE {tag} steps={completed} alive={alive()} issues={len(ISSUES)} -> {out_path}")

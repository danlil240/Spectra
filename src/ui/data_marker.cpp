...
void DataMarkerManager::data_to_screen(float data_x, float data_y,
                                        const Rect& viewport,
...
void DataMarkerManager::draw(const Rect& viewport,
    ...
    for (size_t i = 0; i < markers_.size(); ++i) {
        ...
        float sx, sy;
        data_to_screen(m.data_x, m.data_y, viewport,
                       xlim_min, xlim_max, ylim_min, ylim_max, sx, sy);

        // Skip if outside viewport
        if (sx < viewport.x || sx > viewport.x + viewport.w ||
            sy < viewport.y || sy > viewport.y + viewport.h)
            continue;

        // Outer ring (white/bg)
        ImU32 ring_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
            colors.bg_primary.r, colors.bg_primary.g, colors.bg_primary.b, opacity));
        ...
        ImU32 fill_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
            m.color.r, m.color.g, m.color.b, opacity));
        fg->AddCircleFilled(ImVec2(sx, sy), 5.0f, fill_col);

        // Border
        ImU32 border_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
            colors.border_default.r, colors.border_default.g, colors.border_default.b, opacity * 0.5f));
        ...
        ImU32 bg_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
            colors.bg_elevated.r, colors.bg_elevated.g, colors.bg_elevated.b, 0.92f * opacity));
        ImU32 text_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
            colors.text_primary.r, colors.text_primary.g, colors.text_primary.b, opacity));

        fg->AddRectFilled(
            ImVec2(lx - 4.0f, ly - 2.0f),
...
int DataMarkerManager::hit_test(float screen_x, float screen_y,
                                 const Rect& viewport,
                                 float xlim_min, float xlim_max,
                                 float ylim_min, float ylim_max,
    ...
    for (size_t i = 0; i < markers_.size(); ++i) {
        float sx, sy;
        data_to_screen(markers_[i].data_x, markers_[i].data_y, viewport,
                       xlim_min, xlim_max, ylim_min, ylim_max, sx, sy);

...
void Camera::deserialize(const std::string& json) {
    ...
    if (pos != std::string::npos) position = parse_vec3(json, pos);
    
    pos = json.find("\"target\"");
    if (pos != std::string::npos) target = parse_vec3(json, pos);
    
    pos = json.find("\"up\"");
    if (pos != std::string::npos) up = parse_vec3(json, pos);
    ...
    pos = json.find("\"fov\"");
    if (pos != std::string::npos) fov = parse_float(json, pos);
    
    pos = json.find("\"near_clip\"");
    if (pos != std::string::npos) near_clip = parse_float(json, pos);
    
    pos = json.find("\"far_clip\"");
    if (pos != std::string::npos) far_clip = parse_float(json, pos);
    
    pos = json.find("\"ortho_size\"");
    if (pos != std::string::npos) ortho_size = parse_float(json, pos);
    
    pos = json.find("\"azimuth\"");
    if (pos != std::string::npos) azimuth = parse_float(json, pos);
    
    pos = json.find("\"elevation\"");
    if (pos != std::string::npos) elevation = parse_float(json, pos);
    
    pos = json.find("\"distance\"");
    if (pos != std::string::npos) distance = parse_float(json, pos);
}

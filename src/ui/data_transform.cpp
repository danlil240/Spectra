...
void DataTransform::apply_y(std::span<const float> x_in, std::span<const float> y_in,
                             std::vector<float>& x_out, std::vector<float>& y_out) const {
    switch (type_) {
        case TransformType::Identity:      apply_identity(x_in, y_in, x_out, y_out); break;
        case TransformType::Log10:         apply_log10(x_in, y_in, x_out, y_out); break;
        case TransformType::Ln:            apply_ln(x_in, y_in, x_out, y_out); break;
        case TransformType::Abs:           apply_abs(x_in, y_in, x_out, y_out); break;
        case TransformType::Negate:        apply_negate(x_in, y_in, x_out, y_out); break;
        case TransformType::Normalize:     apply_normalize(x_in, y_in, x_out, y_out); break;
        case TransformType::Standardize:   apply_standardize(x_in, y_in, x_out, y_out); break;
        case TransformType::Derivative:    apply_derivative(x_in, y_in, x_out, y_out); break;
        case TransformType::CumulativeSum: apply_cumulative_sum(x_in, y_in, x_out, y_out); break;
        case TransformType::Diff:          apply_diff(x_in, y_in, x_out, y_out); break;
        case TransformType::Scale:         apply_scale(x_in, y_in, x_out, y_out); break;
        case TransformType::Offset:        apply_offset(x_in, y_in, x_out, y_out); break;
        case TransformType::Clamp:         apply_clamp(x_in, y_in, x_out, y_out); break;
...
float DataTransform::apply_scalar(float value) const {
    if (!is_elementwise()) return std::numeric_limits<float>::quiet_NaN();

    switch (type_) {
        case TransformType::Identity:  return value;
        case TransformType::Log10:     return (value > 0.0f) ? std::log10(value) : std::numeric_limits<float>::quiet_NaN();
        case TransformType::Ln:        return (value > 0.0f) ? std::log(value) : std::numeric_limits<float>::quiet_NaN();
        case TransformType::Abs:       return std::abs(value);
        case TransformType::Negate:    return -value;
        case TransformType::Scale:     return value * params_.scale_factor;
        case TransformType::Offset:    return value + params_.offset_value;
        case TransformType::Clamp:     return std::clamp(value, params_.clamp_min, params_.clamp_max);
        case TransformType::Custom:
            if (custom_func_) return custom_func_(value);
            return value;
        default:
...
std::string DataTransform::description() const {
    switch (type_) {
        case TransformType::Identity:      return "Identity (no change)";
        case TransformType::Log10:         return "Log10(y)";
        case TransformType::Ln:            return "Ln(y)";
        case TransformType::Abs:           return "|y|";
        case TransformType::Negate:        return "-y";
        case TransformType::Normalize:     return "Normalize to [0, 1]";
        case TransformType::Standardize:   return "Standardize (z-score)";
        case TransformType::Derivative:    return "dy/dx";
        case TransformType::CumulativeSum: return "Cumulative sum";
        case TransformType::Diff:          return "First difference";
        case TransformType::Scale: {
...
void DataTransform::apply_identity(std::span<const float> x_in, std::span<const float> y_in,
                                    std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
...
void DataTransform::apply_log10(std::span<const float> x_in, std::span<const float> y_in,
                                 std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.clear();
    y_out.clear();
...
void DataTransform::apply_ln(std::span<const float> x_in, std::span<const float> y_in,
                              std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.clear();
    y_out.clear();
...
void DataTransform::apply_abs(std::span<const float> x_in, std::span<const float> y_in,
                               std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
    y_out.resize(n);
...
void DataTransform::apply_negate(std::span<const float> x_in, std::span<const float> y_in,
                                  std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
    y_out.resize(n);
...
void DataTransform::apply_normalize(std::span<const float> x_in, std::span<const float> y_in,
                                     std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
    y_out.resize(n);
    ...
    for (size_t i = 1; i < n; ++i) {
        if (y_in[i] < ymin) ymin = y_in[i];
        if (y_in[i] > ymax) ymax = y_in[i];
    }
...
void DataTransform::apply_standardize(std::span<const float> x_in, std::span<const float> y_in,
                                       std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
    y_out.resize(n);
    ...
    // Compute mean
    double sum = 0.0;
    for (size_t i = 0; i < n; ++i) sum += static_cast<double>(y_in[i]);
...
void DataTransform::apply_derivative(std::span<const float> x_in, std::span<const float> y_in,
                                      std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    if (n < 2) {
        x_out.clear();
...
void DataTransform::apply_cumulative_sum(std::span<const float> x_in, std::span<const float> y_in,
                                          std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
    y_out.resize(n);
...
void DataTransform::apply_diff(std::span<const float> x_in, std::span<const float> y_in,
                                std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    if (n < 2) {
        x_out.clear();
...
void DataTransform::apply_scale(std::span<const float> x_in, std::span<const float> y_in,
                                 std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
    y_out.resize(n);
...
void DataTransform::apply_offset(std::span<const float> x_in, std::span<const float> y_in,
                                  std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
    y_out.resize(n);
...
void DataTransform::apply_clamp(std::span<const float> x_in, std::span<const float> y_in,
                                 std::vector<float>& x_out, std::vector<float>& y_out) const {
    const size_t n = std::min(x_in.size(), y_in.size());
    x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
    y_out.resize(n);
...
void TransformPipeline::insert(size_t index, const DataTransform& transform) {
    if (index > steps_.size()) index = steps_.size();
...
void TransformPipeline::move_step(size_t from, size_t to) {
    if (from >= steps_.size() || to >= steps_.size() || from == to) return;
...
bool TransformPipeline::is_enabled(size_t index) const {
    if (index < steps_.size()) return steps_[index].enabled;
    return false;
}

void TransformPipeline::apply(std::span<const float> x_in, std::span<const float> y_in,
                               std::vector<float>& x_out, std::vector<float>& y_out) const {
    if (is_identity()) {
        const size_t n = std::min(x_in.size(), y_in.size());
        x_out.assign(x_in.begin(), x_in.begin() + static_cast<std::ptrdiff_t>(n));
...
std::string TransformPipeline::description() const {
    if (steps_.empty()) return "Empty pipeline";
...
void TransformRegistry::register_builtins() {
    ...
    register_transform("reciprocal", [](float v) { return (v != 0.0f) ? (1.0f / v) : 0.0f; }, "1/y");
...
std::vector<float> transform_y(std::span<const float> y, TransformType type,
...
const char* transform_type_name(TransformType type) {
    switch (type) {
        case TransformType::Identity:      return "Identity";
        case TransformType::Log10:         return "Log10";
        case TransformType::Ln:            return "Ln";
        case TransformType::Abs:           return "Abs";
        case TransformType::Negate:        return "Negate";
        case TransformType::Normalize:     return "Normalize";
        case TransformType::Standardize:   return "Standardize";
        case TransformType::Derivative:    return "Derivative";
        case TransformType::CumulativeSum: return "CumulativeSum";
        case TransformType::Diff:          return "Diff";
        case TransformType::Scale:         return "Scale";
        case TransformType::Offset:        return "Offset";
        case TransformType::Clamp:         return "Clamp";
        case TransformType::Custom:        return "Custom";
    }
    return "Unknown";
}

...
TEST_F(PhaseAIntegrationTest, DiscoveryFindsMockPublisher)
{
    ...
    EXPECT_TRUE(disc.has_topic(topic)) << "Expected to discover topic " << topic;
}
...
TEST_F(PhaseAIntegrationTest, FullPipeline_BridgeDiscoverSubscribeExtract)
{
    ...
    std::this_thread::sleep_for(200ms);
    disc.refresh();
    EXPECT_TRUE(disc.has_topic(topic)) << "TopicDiscovery did not find topic " << topic;
...
TEST_F(PhaseAIntegrationTest, TwistMultiFieldExtraction)
{
    ...
    EXPECT_NEAR(s.value, 1.5, 1e-6);
    ASSERT_TRUE(sub.pop(id_ly, s));
    EXPECT_NEAR(s.value, -2.5, 1e-6);
    ASSERT_TRUE(sub.pop(id_az, s));
    EXPECT_NEAR(s.value, 0.75, 1e-6);
...
TEST_F(PhaseAIntegrationTest, ImuLinearAccelerationExtraction)
{
    ...
    FieldSample s;
    ASSERT_TRUE(sub.pop(id_ax, s));
    EXPECT_NEAR(s.value, 9.81, 1e-4);
    ASSERT_TRUE(sub.pop(id_ay, s));
    EXPECT_NEAR(s.value, 0.0, 1e-6);
    ASSERT_TRUE(sub.pop(id_az, s));
...
TEST_F(PhaseAIntegrationTest, TopicAddCallbackFiresDuringFullPipeline)
{
    ...
    EXPECT_TRUE(topic_found.load()) << "Topic add callback was not fired for " << topic;
}

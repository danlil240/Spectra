...
TEST(CVDSimulation, BlackRemainsBlack) {
    Color black(0.0f, 0.0f, 0.0f);
    for (auto type : {CVDType::Protanopia, CVDType::Deuteranopia, CVDType::Tritanopia, CVDType::Achromatopsia}) {
...
protected:
    ...
    void TearDown() override {
        auto& tm = ThemeManager::instance();
        if (tm.is_transitioning()) tm.update(10.0f);
        if (tm.is_palette_transitioning()) tm.update(10.0f);
...
TEST_F(ColorblindPaletteTest, AllPaletteColorsAreDistinct) {
    ...
    for (const auto& name : tm.available_data_palettes()) {
        ...
        for (size_t i = 0; i + 1 < colors.size(); ++i) {
            bool same = (colors[i].r == colors[i + 1].r &&
                         colors[i].g == colors[i + 1].g &&
                         colors[i].b == colors[i + 1].b);
            EXPECT_FALSE(same) << "Palette '" << name << "' colors " << i << " and " << i + 1 << " are identical";
        }
    }
}
...
TEST_F(ColorblindPaletteTest, AllPalettesHaveDescriptions) {
    ...
    for (const auto& name : tm.available_data_palettes()) {
        const auto& pal = tm.get_data_palette(name);
        if (!pal.name.empty()) {
            EXPECT_FALSE(pal.description.empty())
                << "Palette '" << name << "' has no description";
        }
    }
}
...
TEST_F(ColorblindPaletteTest, CVDSafeColorsRemainDistinguishable) {
    ...
    for (const auto& name : tm.available_data_palettes()) {
        const auto& pal = tm.get_data_palette(name);
        if (!pal.colorblind_safe) continue;
...
TEST_F(ColorblindPaletteTest, RegisterCustomPalette) {
    ...
    for (const auto& n : names) {
        if (n == "custom_test_pal") { found = true; break; }
    }
    EXPECT_TRUE(found);
...
TEST_F(ColorblindPaletteTest, TransitionChainDoesNotCorrupt) {
    ...
    tm.set_theme("dark");
    float lum = 0.2126f * tm.colors().bg_primary.r + 0.7152f * tm.colors().bg_primary.g + 0.0722f * tm.colors().bg_primary.b;
    EXPECT_LT(lum, 0.15f);
}

...
static ImVec4 topic_type_color(const char* type_name)
{
    ...
    if (pkg == "rcl_interfaces")  return {0.60f, 0.60f, 0.60f, 1.0f};
...
static void draw_hz_sparkline(const float* history, int n, float width, float height)
{
    ...
    float max_val = 0.0f;
    for (int i = 0; i < n; ++i) if (history[i] > max_val) max_val = history[i];
    if (max_val <= 0.0f) { ImGui::Dummy({width, height}); return; }
...
struct MockLogEntry { int sev; const char* node; const char* msg; };
...
static void draw_log_viewer_panel()
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        ImVec4 btn_col = g_pills[i]
            ? ImVec4(col.x*0.45f, col.y*0.45f, col.z*0.45f, 0.9f)
            : ImVec4(0.18f, 0.18f, 0.18f, 0.7f);
        ImGui::PushStyleColor(ImGuiCol_Button,        btn_col);
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(col.x*0.65f, col.y*0.65f, col.z*0.65f, 1.0f));
        ImGui::PushStyleColor(ImGuiCol_Text,          g_pills[i] ? col : ImVec4(0.4f,0.4f,0.4f,1.0f));
        char id[24]; std::snprintf(id, sizeof(id), "%s##pill%d", kPillLabels[i], i);
        if (ImGui::SmallButton(id)) g_pills[i] = !g_pills[i];
        ImGui::PopStyleColor(3);
        if (i < 4) ImGui::SameLine(0.0f, 3.0f);
    }
    ...
    ImGui::SameLine();
    if (ImGui::SmallButton("X##clr")) node_buf[0] = '\0';
    ImGui::Separator();

    if (ImGui::BeginTable("logtbl", 3,
        ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY,
        {0, 180}))
    {
        ...
        for (auto& e : g_log)
        {
            if (!g_pills[e.sev]) continue;  // hidden by pill toggle
            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
...
struct MockDiagComp { const char* name; int level; float stale_sec; };
...
static void draw_diagnostics_panel()
{
    ...
    // Summary badges
    int counts[4] = {};
    for (auto& c : g_diag) if (c.level >= 0 && c.level < 4) counts[c.level]++;
    for (int i = 0; i < 4; ++i)
    {
        ImGui::TextColored(kDiagColors[i], "%s: %d", kDiagLabels[i], counts[i]);
        if (i < 3) ImGui::SameLine(0.0f, 16.0f);
    }
    ImGui::Separator();

    if (ImGui::BeginTable("diagtbl", 3,
        ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY,
        {0, 200}))
    {
...
struct MockEvent { float pos; std::array<float,4> color; const char* tooltip; };
...
static void draw_bag_playback_panel()
{
    ...
    for (auto& ev : g_events)
    {
        ...
        ImU32 col = IM_COL32(
            (int)(ev.color[0]*255), (int)(ev.color[1]*255),
            (int)(ev.color[2]*255), (int)(ev.color[3]*255));
    ...
    // Playhead cursor
    dl->AddLine({fill_x, bar_min.y - 2}, {fill_x, bar_max.y + 2},
...
static void draw_subplot_context_menu_panel()
{
    ...
    for (int s = 0; s < 3; ++s)
    {
        ...
        char popup_id[32]; std::snprintf(popup_id, sizeof(popup_id), "##slotmenu%d", s);
...
static void draw_expression_editor_panel()
{
    ...
    if (ac_open)
    {
        ...
        ImGui::SetNextWindowPos({ImGui::GetItemRectMin().x,
                                 ImGui::GetItemRectMax().y + 2.0f});
        ImGui::SetNextWindowSize({400, 160});
        ImGui::SetNextWindowBgAlpha(0.95f);
        if (ImGui::Begin("##acpopup", nullptr,
            ...
            ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing))
        {
            ...
            {
                if (strstr(kFieldEntries[i], prefix + 1) == nullptr) continue;
                bool sel = (visible == ac_sel);
                if (sel) ImGui::PushStyleColor(ImGuiCol_Text, {0.3f,0.9f,1.0f,1.0f});
            ...
            if (visible == 0) { ImGui::TextDisabled("(no matches)"); ac_open = false; }
        }
        ImGui::End();
    }
    ...
    ImGui::SameLine();
    if (ImGui::SmallButton("Close autocomplete")) ac_open = !ac_open;

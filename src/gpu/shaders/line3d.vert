...
// Vertex data via SSBO (set 1, binding 0) — 3D points stored as vec4 (xyz + padding)
...
void main() {
    ...
    // Fetch the two endpoints of this segment (3D points)
    ...
    // Cap max extrusion to avoid artifacts at extreme perspective
    float half_width = min(line_width * 0.5 + 1.0, 50.0);
    ...
    // Interpolate depth properly between the two endpoints
    float depth = mix(clip0.z, clip1.z, along);
    float w = mix(clip0.w, clip1.w, along);

    gl_Position = vec4(ndc * w, depth, w);
    ...
    // lengths. Cap the walk at 512 segments to bound GPU cost.
    float cumul = 0.0;

...
int main()
{
    ...
    auto& scatter_series = spectra::scatter(x_anim, y_anim).color(spectra::rgb(1.0f, 0.65f, 0.0f)).label("Particles");
    ...
    for (size_t i = 0; i < M; ++i)
    {
        ...
        y4[i] = 0.3f * std::sin(x * 5.0f) + 0.2f * std::cos(x * 3.0f); // High frequency components
    }
    ...
    spectra::histogram(normal_data, 30).color(spectra::rgb(0.3f, 0.6f, 1.0f)).label("Normal Distribution");

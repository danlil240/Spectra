...
std::vector<SelectedPoint> collect_points(const Axes& axes,
                                           float sel_xmin, float sel_xmax,
                                           float sel_ymin, float sel_ymax) {
...
RegionStatistics compute_statistics(const std::vector<SelectedPoint>& points) {
    ...
    if (stats.point_count > 1) {
        ...
        stats.y_std = static_cast<float>(std::sqrt(sum_sq / static_cast<double>(stats.point_count - 1)));
    }
    return stats;
}

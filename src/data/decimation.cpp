...
std::vector<std::pair<float, float>> lttb(
    ...
    std::size_t target_count)
{
    ...
    for (std::size_t bucket = 0; bucket < target_count - 2; ++bucket) {
        ...
        const auto bucket_start = static_cast<std::size_t>(
            std::floor(static_cast<double>(bucket) * bucket_size)) + 1;
        const auto bucket_end = static_cast<std::size_t>(
            std::floor(static_cast<double>(bucket + 1) * bucket_size)) + 1;

        // Next bucket range (for computing the average point)
        const auto next_start = static_cast<std::size_t>(
            std::floor(static_cast<double>(bucket + 1) * bucket_size)) + 1;
        const auto next_end = (bucket + 2 < target_count - 1)
            ? static_cast<std::size_t>(
                  std::floor(static_cast<double>(bucket + 2) * bucket_size)) + 1
            : n;
        ...
        for (std::size_t i = bucket_start; i < bucket_end && i < n; ++i) {
            ...
            const double area = std::abs(
                (px - avg_x) * (static_cast<double>(y[i]) - py) -
...
std::vector<std::pair<float, float>> min_max_decimate(
    std::span<const float> x,
    std::span<const float> y,
    std::size_t bucket_count)
{
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        if (bi >= bucket_count) bi = bucket_count - 1;

        auto& b = buckets[bi];
        b.has_data = true;
        if (y[i] < b.min_y) { b.min_y = y[i]; b.min_x = x[i]; b.min_idx = i; }
        if (y[i] > b.max_y) { b.max_y = y[i]; b.max_x = x[i]; b.max_idx = i; }
    }
...
std::vector<std::pair<float, float>> resample_uniform(
    std::span<const float> x,
    std::span<const float> y,
    std::size_t output_count)
{
    ...
    const float step = (output_count > 1)
        ? (x_end - x_start) / static_cast<float>(output_count - 1)
        : 0.0f;

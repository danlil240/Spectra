...
static void BM_AxisLink_PropagateFrom(benchmark::State& state) {
    ...
    for (auto& ax : axes) { ax.xlim(0, 10); ax.ylim(0, 10); }

    auto gid = mgr.create_group("Linked", LinkAxis::Both);
    for (auto& ax : axes) { mgr.add_to_group(gid, &ax); }
...
BENCHMARK(BM_AxisLink_PropagateFrom)->Arg(2)->Arg(5)->Arg(10)->Arg(20)->Unit(benchmark::kMicrosecond);
...
static void BM_AxisLink_Serialization(benchmark::State& state) {
    ...
    for (int i = 0; i < 3; ++i) mgr.add_to_group(g1, &axes[i]);
    for (int i = 3; i < 6; ++i) mgr.add_to_group(g2, &axes[i]);

    auto mapper = [&](const Axes* a) -> int {
        for (int i = 0; i < 6; ++i) if (&axes[i] == a) return i;
        return -1;
    };
...
BENCHMARK(BM_DataTransform_SingleApply)->Arg(100)->Arg(1000)->Arg(10000)->Arg(100000)->Unit(benchmark::kMicrosecond);
...
BENCHMARK(BM_DataTransform_Pipeline3Steps)->Arg(100)->Arg(1000)->Arg(10000)->Arg(100000)->Unit(benchmark::kMicrosecond);
...
BENCHMARK(BM_DataTransform_Derivative)->Arg(1000)->Arg(10000)->Arg(100000)->Unit(benchmark::kMicrosecond);
...
BENCHMARK(BM_DataTransform_Normalize)->Arg(1000)->Arg(10000)->Arg(100000)->Unit(benchmark::kMicrosecond);
...
static void BM_KeyframeInterp_EvaluateLinear(benchmark::State& state) {
    ...
    for (int i = 0; i < num_kf; ++i) {
        ch.add_keyframe(TypedKeyframe(static_cast<float>(i), static_cast<float>(i) * 0.5f,
                                       InterpMode::Linear));
    }
...
BENCHMARK(BM_KeyframeInterp_EvaluateLinear)->Arg(10)->Arg(50)->Arg(200)->Arg(1000)->Unit(benchmark::kNanosecond);

static void BM_KeyframeInterp_EvaluateCubicBezier(benchmark::State& state) {
    AnimationChannel ch("bench", 0.0f);
    for (int i = 0; i < 20; ++i) {
        TypedKeyframe kf(static_cast<float>(i), static_cast<float>(i) * 0.5f, InterpMode::CubicBezier);
...
static void BM_KeyframeInterp_SampleChannel(benchmark::State& state) {
    AnimationChannel ch("bench", 0.0f);
    for (int i = 0; i < 10; ++i) {
        ch.add_keyframe(TypedKeyframe(static_cast<float>(i), std::sin(static_cast<float>(i)),
                                       InterpMode::Linear));
    }
...
BENCHMARK(BM_KeyframeInterp_SampleChannel)->Arg(100)->Arg(500)->Arg(2000)->Unit(benchmark::kMicrosecond);
...
BENCHMARK(BM_KeyframeInterp_EvaluateAll)->Arg(1)->Arg(5)->Arg(20)->Arg(50)->Unit(benchmark::kMicrosecond);
...
static void BM_KeyframeInterp_Serialization(benchmark::State& state) {
    ...
    for (int i = 0; i < 5; ++i) {
        auto ch_id = interp.add_channel("ch_" + std::to_string(i));
        for (int j = 0; j < 20; ++j) {
            interp.add_keyframe(ch_id, TypedKeyframe(static_cast<float>(j),
...
static void BM_ShortcutConfig_SetOverride(benchmark::State& state) {
    ...
    for (auto _ : state) {
        config.set_override("cmd." + std::to_string(i % 100), "Ctrl+" + std::to_string(i % 26 + 65));
        ++i;
    }
...
static WorkspaceData make_phase3_workspace(int num_figures, int series_per_fig) {
    ...
    for (int f = 0; f < num_figures; ++f) {
        ...
        fig.width = 1280; fig.height = 720;
        fig.custom_tab_title = "Tab " + std::to_string(f + 1);

        WorkspaceData::AxisState ax;
        ax.x_min = 0; ax.x_max = 10; ax.y_min = -1; ax.y_max = 1;
        ax.title = "Axes"; ax.x_label = "X"; ax.y_label = "Y";

...
TEST_F(ThemeManagerTest, AvailablePalettesContainsDefaults) {
    ...
    for (const auto& n : names) {
        ...
        if (n == "colorblind") has_colorblind = true;
    }
...
TEST_F(ThemeManagerTest, AllThemesHavePlotColors) {
    ...
    for (const auto& name : {"dark", "light", "high_contrast"}) {
        tm.set_theme(name);
        const auto& c = tm.colors();
        float sum = c.grid_line.r + c.grid_line.g + c.grid_line.b +
                    c.axis_line.r + c.axis_line.g + c.axis_line.b +
                    c.tick_label.r + c.tick_label.g + c.tick_label.b;

...
NearestPointResult find_nearest_standalone(
    float cursor_screen_x, float cursor_screen_y,
    const Axes& axes, const Rect& viewport)
{
...
TEST_F(DataInteractionTest, NearestPointFindsExactMatch) {
    ...
    auto result = find_nearest_standalone(500.0f, 100.0f, *axes_,
                                          axes_->viewport());
...
TEST_F(DataInteractionTest, NearestPointFindsClosest) {
    ...
    auto result = find_nearest_standalone(305.0f, 405.0f, *axes_,
                                          axes_->viewport());
    ASSERT_TRUE(result.found);
...
TEST_F(DataInteractionTest, NearestPointSnapsToActualData) {
    ...
    // (5,10)  -> screen (500, 100)
    // Midpoint: (400, 250)
    auto result = find_nearest_standalone(400.0f, 250.0f, *axes_,
                                          axes_->viewport());
    ASSERT_TRUE(result.found);
...
TEST_F(DataInteractionTest, NearestPointEmptySeriesReturnsNotFound) {
    ...
    auto result = find_nearest_standalone(400.0f, 300.0f, empty_axes,
                                          empty_axes.viewport());
...
TEST_F(DataInteractionTest, NearestPointScatterSeries) {
    ...
    auto result = find_nearest_standalone(500.0f, 500.0f, scatter_axes,
                                          scatter_axes.viewport());
    ASSERT_TRUE(result.found);
    EXPECT_EQ(result.point_index, 1u);
...
TEST_F(DataInteractionTest, NearestPointFirstAndLastPoints) {
    ...
    auto result = find_nearest_standalone(100.0f, 700.0f, *axes_,
                                          axes_->viewport());
    ASSERT_TRUE(result.found);
    ...
    result = find_nearest_standalone(900.0f, 700.0f, *axes_,
                                     axes_->viewport());
    ASSERT_TRUE(result.found);
...
void data_to_screen(float data_x, float data_y,
                    const Rect& viewport,
...
int marker_hit_test(const std::vector<TestMarker>& markers,
    ...
    for (size_t i = 0; i < markers.size(); ++i) {
        float sx, sy;
        data_to_screen(markers[i].data_x, markers[i].data_y, viewport,
...
TEST(DataMarkerTest, HitTestFindsMarker) {
    ...
    int idx = marker_hit_test(markers, 502.0f, 498.0f, vp,
...
TEST(DataMarkerTest, HitTestMissesDistantClick) {
    ...
    int idx = marker_hit_test(markers, 100.0f, 100.0f, vp,
                              0.0f, 100.0f, 0.0f, 100.0f);
    EXPECT_EQ(idx, -1);
}
...
TEST(DataMarkerTest, HitTestMultipleMarkers) {
    ...
    int idx = marker_hit_test(markers, 901.0f, 101.0f, vp,
                              0.0f, 100.0f, 0.0f, 100.0f);
    EXPECT_EQ(idx, 1);
}

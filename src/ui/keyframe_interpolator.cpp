...
const char* interp_mode_name(InterpMode mode) {
    switch (mode) {
        ...
        case InterpMode::Spring:      return "Spring";
        case InterpMode::EaseIn:      return "EaseIn";
        case InterpMode::EaseOut:     return "EaseOut";
        case InterpMode::EaseInOut:   return "EaseInOut";
    }
    return "Unknown";
}
...
const char* tangent_mode_name(TangentMode mode) {
    switch (mode) {
        case TangentMode::Free:    return "Free";
        case TangentMode::Aligned: return "Aligned";
        case TangentMode::Flat:    return "Flat";
        case TangentMode::Auto:    return "Auto";
    }
    return "Unknown";
}
...
bool AnimationChannel::set_keyframe_tangents(float time, TangentHandle in, TangentHandle out,
                                              float tolerance) {
    auto* kf = find_keyframe(time, tolerance);
    if (!kf) return false;
    kf->in_tangent = in;
...
TypedKeyframe* AnimationChannel::find_keyframe(float time, float tolerance) {
    for (auto& kf : keyframes_) {
        if (std::abs(kf.time - time) < tolerance) return &kf;
    }
    return nullptr;
}
...
float AnimationChannel::evaluate(float time) const {
    if (keyframes_.empty()) return default_value_;
    ...
    for (size_t i = 0; i + 1 < keyframes_.size(); ++i) {
        ...
        if (time >= a.time && time <= b.time) {
            ...
            if (segment_duration <= 0.0f) return a.value;

            float t = (time - a.time) / segment_duration;

            switch (a.interp) {
                case InterpMode::Step:        return interp_step(a, b, t);
                case InterpMode::Linear:      return interp_linear(a, b, t);
                case InterpMode::CubicBezier: return interp_cubic_bezier(a, b, t);
                case InterpMode::Spring:      return interp_spring(a, b, t);
                case InterpMode::EaseIn:      return interp_ease_in(a, b, t);
                case InterpMode::EaseOut:     return interp_ease_out(a, b, t);
                case InterpMode::EaseInOut:   return interp_ease_in_out(a, b, t);
            }
        }
    }
...
float AnimationChannel::interp_spring(const TypedKeyframe& a, const TypedKeyframe& b, float t) {
    ...
    float spring_t = 1.0f - decay * (std::cos(omega_d * t) +
...
void KeyframeInterpolator::remove_channel(uint32_t channel_id) {
    std::lock_guard lock(mutex_);
    std::erase_if(channels_, [channel_id](const auto& pair) {
        return pair.first == channel_id;
    });
    // Also remove bindings for this channel
    std::erase_if(bindings_, [channel_id](const PropertyBinding& b) {
        return b.channel_id == channel_id;
    });
}
...
void KeyframeInterpolator::bind(uint32_t channel_id, const std::string& prop_name,
                                 float* target, float scale, float offset) {
    std::lock_guard lock(mutex_);
    PropertyBinding b;
...
void KeyframeInterpolator::bind_color(uint32_t channel_id, const std::string& prop_name,
                                       Color* target) {
    std::lock_guard lock(mutex_);
    PropertyBinding b;
    b.channel_id = channel_id;
...
void KeyframeInterpolator::bind_callback(uint32_t channel_id, const std::string& prop_name,
    ...
    for (size_t ci = 0; ci < channels_.size(); ++ci) {
        ...
        for (size_t ki = 0; ki < kfs.size(); ++ki) {
            ...
            ss << "{\"t\":" << kf.time
               ...
               << ",\"tm\":" << static_cast<int>(kf.tangent_mode)
               << ",\"it\":[" << kf.in_tangent.dt << "," << kf.in_tangent.dv << "]"
...
bool KeyframeInterpolator::deserialize(const std::string& json) {
    ...
    if (json.find("\"channels\"") == std::string::npos) return false;
    ...
    while (pos < json.size()) {
        if (json[pos] == '{') {
            ...
            if (depth == 0) {
                ...
                if (kf_pos != std::string::npos) {
                    ...
                    if (kf_arr != std::string::npos) {
                        ...
                        while (kf_arr < block.size()) {
                            if (block[kf_arr] == '{') {
                                if (kf_depth == 0) kf_start = kf_arr;
                                kf_depth++;
                            } else if (block[kf_arr] == '}') {
                                kf_depth--;
                                if (kf_depth == 0) {
                                    std::string kf_block = block.substr(kf_start, kf_arr - kf_start + 1);

                                    TypedKeyframe kf;
                                    kf.time = extract_float(kf_block, "t");
                                    kf.value = extract_float(kf_block, "v");
                                    kf.interp = static_cast<InterpMode>(extract_int(kf_block, "i"));
                                    kf.tangent_mode = static_cast<TangentMode>(extract_int(kf_block, "tm"));

                                    // Parse tangent arrays [dt, dv]
                                    auto parse_tangent = [&](const std::string& key) -> TangentHandle {
                                        std::string search = "\"" + key + "\":[";
                                        ...
                                        if (comma == std::string::npos || bracket == std::string::npos) return {};
                                        try {
                                            float dt = std::stof(kf_block.substr(tpos, comma - tpos));
                                            float dv = std::stof(kf_block.substr(comma + 1, bracket - comma - 1));
                ...
                if (id >= next_channel_id_) next_channel_id_ = id + 1;

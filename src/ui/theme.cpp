...
const ThemeColors& ThemeManager::colors() const {
    if (display_colors_valid_) return display_colors_;
...
const DataPalette& ThemeManager::current_data_palette() const {
    ...
    if (display_palette_valid_) return display_palette_;
    if (current_theme_) {
...
const DataPalette& ThemeManager::get_data_palette(const std::string& name) const {
    ...
    if (it != data_palettes_.end()) return it->second;
    return fallback;
}
...
void ThemeManager::transition_palette(const std::string& palette_name, float duration_sec) {
    auto it = data_palettes_.find(palette_name);
    if (it == data_palettes_.end()) return;
...
void ThemeManager::update(float dt) {
    ...
    if (transitioning_) {
        ...
        display_colors_ = interpolate_colors(transition_start_colors_, transition_target_colors_, t);
        display_colors_valid_ = true;
        apply_to_imgui();
    ...
    if (palette_transitioning_) {
        ...
        display_palette_.colors.resize(std::max(palette_start_colors_.size(), palette_target_colors_.size()));
        for (size_t i = 0; i < count; ++i) {
            display_palette_.colors[i] = palette_start_colors_[i].lerp(palette_target_colors_[i], t);
        }
...
std::string theme_colors_to_json(const ThemeColors& c, int indent = 4) {
    ...
    os << "{\n";
    #define TC_FIELD(name) os << pad << "  " << escape_json_string(#name) << ": " << color_to_json(c.name)
    TC_FIELD(bg_primary) << ",\n";
...
bool parse_theme_colors_from_json(const std::string& json, ThemeColors& out) {
    #define PARSE_TC(name) { \
        ...
        if (p != std::string::npos) { p += strlen("\"" #name "\""); out.name = parse_color_array(json, p); } \
    }
    PARSE_TC(bg_primary); PARSE_TC(bg_secondary); PARSE_TC(bg_tertiary);
    PARSE_TC(bg_elevated); PARSE_TC(bg_overlay);
    PARSE_TC(text_primary); PARSE_TC(text_secondary); PARSE_TC(text_tertiary); PARSE_TC(text_inverse);
    PARSE_TC(border_default); PARSE_TC(border_subtle); PARSE_TC(border_strong);
    PARSE_TC(accent); PARSE_TC(accent_hover); PARSE_TC(accent_muted); PARSE_TC(accent_subtle);
    PARSE_TC(success); PARSE_TC(warning); PARSE_TC(error); PARSE_TC(info);
    PARSE_TC(grid_line); PARSE_TC(axis_line); PARSE_TC(tick_label); PARSE_TC(crosshair);
    PARSE_TC(selection_fill); PARSE_TC(selection_border); PARSE_TC(tooltip_bg); PARSE_TC(tooltip_border);
    #undef PARSE_TC
    return true;
}
...
bool ThemeManager::export_theme(const std::string& path) const {
    if (!current_theme_) return false;
    ...
    for (size_t i = 0; i < t.data_palette.colors.size(); ++i) {
        ...
        if (i + 1 < t.data_palette.colors.size()) f << ",";
        f << "\n";
    }
    f << "    ]\n";
    f << "  }\n";
...
void ThemeManager::load_default() {
    ...
    if (!default_theme_path_.empty() && std::filesystem::exists(default_theme_path_)) {
        if (import_theme(default_theme_path_)) {
            ...
            std::string json((std::istreambuf_iterator<char>(lf)), std::istreambuf_iterator<char>());
            size_t np = json.find("\"name\"");
            if (np != std::string::npos) {
                std::string n = extract_string_value(json, np + 6);
                if (!n.empty()) set_theme(n);
            }
        }
    }
}
...
void ThemeManager::initialize_data_palettes() {
    ...
    mono.safe_for = {CVDType::Protanopia, CVDType::Deuteranopia, CVDType::Tritanopia, CVDType::Achromatopsia};
    mono.colors = {
        Color::from_hex(0x000000),  // black

...
static void scene_line_styles(App& /*app*/, Figure& fig) {
    ...
    for (size_t i = 0; i < N; ++i) x[i] = static_cast<float>(i) * 0.05f;

    // Five series with different line styles
    struct StyleDef { const char* label; LineStyle ls; float offset; Color color; };
    StyleDef styles[] = {
...
static void scene_marker_styles(App& /*app*/, Figure& fig) {
    ...
    MarkerStyle markers[] = {
        MarkerStyle::Circle, MarkerStyle::Square, MarkerStyle::Diamond,
        MarkerStyle::TriangleUp, MarkerStyle::Star, MarkerStyle::Plus,
        MarkerStyle::Cross, MarkerStyle::Pentagon, MarkerStyle::Hexagon,
    };
    Color marker_colors[] = {
        rgb(0.2f, 0.6f, 1.0f), rgb(1.0f, 0.4f, 0.2f), rgb(0.3f, 0.9f, 0.4f),
        rgb(0.9f, 0.2f, 0.8f), rgb(0.8f, 0.7f, 0.1f), rgb(0.1f, 0.8f, 0.8f),
        rgb(0.6f, 0.3f, 0.9f), rgb(0.9f, 0.6f, 0.3f), rgb(0.4f, 0.4f, 0.9f),
    };
...
static void scene_filled_markers(App& /*app*/, Figure& fig) {
    ...
    for (int m = 0; m < 4; ++m) {
        ...
        auto& s = ax.scatter(x, y)
            .label(marker_style_name(markers[m]))
            .color(rgb(0.2f + 0.2f * static_cast<float>(m), 0.5f, 0.9f - 0.2f * static_cast<float>(m)))
            .size(10.0f);
        s.marker_style(markers[m]);
    }
...
static void scene_line_with_markers(App& /*app*/, Figure& fig) {
    ...
    std::vector<float> x(N);
    for (size_t i = 0; i < N; ++i) x[i] = static_cast<float>(i) * 0.33f;

    // Dashed line with circle markers
    {
        std::vector<float> y(N);
        for (size_t i = 0; i < N; ++i) y[i] = std::sin(x[i]);
        auto& s = ax.line(x, y).label("sin(x) dashed+circle").color(rgb(0.2f, 0.6f, 1.0f)).width(2.0f);
    ...
    // Dotted line with square markers
    {
        ...
        auto& s = ax.line(x, y).label("cos(x) dotted+square").color(rgb(1.0f, 0.4f, 0.2f)).width(2.0f);
    ...
    {
        std::vector<float> y(N);
        for (size_t i = 0; i < N; ++i) y[i] = std::sin(x[i] * 0.5f) * 0.7f;
        auto& s = ax.line(x, y).label("slow sin dash-dot+diamond").color(rgb(0.3f, 0.9f, 0.3f)).width(2.0f);
...
static void scene_styled_subplots(App& /*app*/, Figure& fig) {
    ...
    std::vector<float> x(N);
    for (size_t i = 0; i < N; ++i) x[i] = static_cast<float>(i) * 0.1f;
    ...
    // Top-left: solid blue
    {
        ...
        ax.xlim(0, 10); ax.ylim(-1.5f, 1.5f);
        ax.title("Solid"); ax.grid(true);
    }
    ...
    // Bottom-right: scatter only
    {
        ...
        ax.xlim(0, 10); ax.ylim(-2, 2);
        ax.title("Scatter Stars"); ax.grid(true);
    }
}
...
static void scene_dense_styled(App& /*app*/, Figure& fig) {
    ...
    std::vector<float> x(N);
    for (size_t i = 0; i < N; ++i) x[i] = static_cast<float>(i) * 0.067f;

    LineStyle line_styles[] = {LineStyle::Solid, LineStyle::Dashed, LineStyle::Dotted,
                               LineStyle::DashDot, LineStyle::DashDotDot};
    Color line_colors[] = {
        rgb(0.2f, 0.6f, 1.0f), rgb(1.0f, 0.4f, 0.2f), rgb(0.3f, 0.9f, 0.4f),
        rgb(0.9f, 0.2f, 0.8f), rgb(0.8f, 0.7f, 0.1f), rgb(0.1f, 0.8f, 0.8f),
        rgb(0.6f, 0.3f, 0.9f), rgb(0.9f, 0.6f, 0.3f),
    };
    ...
    for (int s_idx = 0; s_idx < 8; ++s_idx) {
        ...
        auto& s = ax.line(x, y)
            .label("s" + std::to_string(s_idx))
            .color(line_colors[s_idx])
            .width(2.0f);
...
static void scene_format_strings(App& /*app*/, Figure& fig) {
    ...
    std::vector<float> x(N);
    for (size_t i = 0; i < N; ++i) x[i] = static_cast<float>(i) * 0.167f;

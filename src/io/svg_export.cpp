...
void emit_grid(std::ostringstream& svg, const Axes& axes, const DataToSvg& m) {
    ...
    for (float tx : x_ticks.positions) {
        float sx = m.map_x(tx);
        svg << "      <line x1=\"" << fmt(sx) << "\" y1=\"" << fmt(m.vp_y)
            << "\" x2=\"" << fmt(sx) << "\" y2=\"" << fmt(m.vp_y + m.vp_h)
            << "\"/>\n";
    }
    ...
    for (float ty : y_ticks.positions) {
        float sy = m.map_y(ty);
        svg << "      <line x1=\"" << fmt(m.vp_x) << "\" y1=\"" << fmt(sy)
            << "\" x2=\"" << fmt(m.vp_x + m.vp_w) << "\" y2=\"" << fmt(sy)
            << "\"/>\n";
    }
...
void emit_border(std::ostringstream& svg, const DataToSvg& m) {
    svg << "    <rect x=\"" << fmt(m.vp_x) << "\" y=\"" << fmt(m.vp_y)
        << "\" width=\"" << fmt(m.vp_w) << "\" height=\"" << fmt(m.vp_h)
...
void emit_tick_labels(std::ostringstream& svg, const Axes& axes, const DataToSvg& m) {
    ...
    svg << "    <g class=\"tick-labels\" font-family=\"sans-serif\" "
           "font-size=\"" << fmt(font_size) << "\" fill=\"#333\">\n";
    ...
    for (size_t i = 0; i < x_ticks.positions.size(); ++i) {
        ...
        // Tick mark
        svg << "      <line x1=\"" << fmt(sx) << "\" y1=\"" << fmt(bottom)
            << "\" x2=\"" << fmt(sx) << "\" y2=\"" << fmt(bottom + tick_len)
            << "\" stroke=\"#000\" stroke-width=\"1\"/>\n";
        // Label
        svg << "      <text x=\"" << fmt(sx) << "\" y=\"" << fmt(bottom + label_offset)
            << "\" text-anchor=\"middle\">"
            << xml_escape(x_ticks.labels[i]) << "</text>\n";
    }
    ...
    for (size_t i = 0; i < y_ticks.positions.size(); ++i) {
        ...
        // Tick mark
        svg << "      <line x1=\"" << fmt(m.vp_x - tick_len) << "\" y1=\"" << fmt(sy)
            << "\" x2=\"" << fmt(m.vp_x) << "\" y2=\"" << fmt(sy)
            << "\" stroke=\"#000\" stroke-width=\"1\"/>\n";
        // Label
        svg << "      <text x=\"" << fmt(m.vp_x - tick_len - 3.0f) << "\" y=\"" << fmt(sy + 3.5f)
            << "\" text-anchor=\"end\">"
            << xml_escape(y_ticks.labels[i]) << "</text>\n";
    }
...
void emit_labels(std::ostringstream& svg, const Axes& axes, const DataToSvg& m) {
    ...
    if (!axes.get_title().empty()) {
        ...
        svg << "    <text x=\"" << fmt(cx) << "\" y=\"" << fmt(ty)
            << "\" text-anchor=\"middle\" font-family=\"sans-serif\" "
               "font-size=\"" << fmt(title_font) << "\" font-weight=\"bold\" fill=\"#000\">"
    ...
    if (!axes.get_xlabel().empty()) {
        ...
        svg << "    <text x=\"" << fmt(cx) << "\" y=\"" << fmt(ly)
            << "\" text-anchor=\"middle\" font-family=\"sans-serif\" "
               "font-size=\"" << fmt(label_font) << "\" fill=\"#333\">"
            << xml_escape(axes.get_xlabel()) << "</text>\n";
    }
    ...
    if (!axes.get_ylabel().empty()) {
        ...
        svg << "    <text x=\"" << fmt(lx) << "\" y=\"" << fmt(cy)
            << "\" text-anchor=\"middle\" font-family=\"sans-serif\" "
               "font-size=\"" << fmt(label_font) << "\" fill=\"#333\" "
               "transform=\"rotate(-90," << fmt(lx) << "," << fmt(cy) << ")\">"
            << xml_escape(axes.get_ylabel()) << "</text>\n";
    }
}
...
void emit_line_series(std::ostringstream& svg, const LineSeries& series, const DataToSvg& m) {
    if (series.point_count() < 2) return;

    auto x = series.x_data();
    auto y = series.y_data();
    const auto& c = static_cast<const Series&>(series).color();

    svg << "    <polyline fill=\"none\" stroke=\"" << svg_color(c)
        << "\" stroke-width=\"" << fmt(series.width())
        << "\" stroke-opacity=\"" << fmt(c.a)
...
void emit_scatter_series(std::ostringstream& svg, const ScatterSeries& series, const DataToSvg& m) {
    if (series.point_count() == 0) return;
    ...
    svg << "    <g fill=\"" << svg_color(c)
        << "\" fill-opacity=\"" << fmt(c.a) << "\">\n";

    for (size_t i = 0; i < series.point_count(); ++i) {
        svg << "      <circle cx=\"" << fmt(m.map_x(x[i]))
            << "\" cy=\"" << fmt(m.map_y(y[i]))
            << "\" r=\"" << fmt(r) << "\"/>\n";
    }
...
void emit_legend(std::ostringstream& svg, const Axes& axes, const DataToSvg& m) {
    ...
    // Background box
    svg << "    <rect x=\"" << fmt(lx) << "\" y=\"" << fmt(ly)
        << "\" width=\"" << fmt(legend_w) << "\" height=\"" << fmt(legend_h)
        << "\" fill=\"white\" fill-opacity=\"0.9\" stroke=\"#ccc\" stroke-width=\"1\" rx=\"3\"/>\n";

    svg << "    <g font-family=\"sans-serif\" font-size=\"" << fmt(font_size) << "\" fill=\"#333\">\n";
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
        ...
        if (entries[i].is_line) {
            // Line swatch
            svg << "      <line x1=\"" << fmt(ex) << "\" y1=\"" << fmt(ey)
                << "\" x2=\"" << fmt(ex + swatch_w) << "\" y2=\"" << fmt(ey)
                << "\" stroke=\"" << svg_color(entries[i].color)
                << "\" stroke-width=\"2\"/>\n";
        } else {
            // Circle swatch

...
void ShortcutConfig::set_override(const std::string& command_id,
...
void ShortcutConfig::remove_override(const std::string& command_id) {
    std::erase_if(overrides_, [&](const BindingOverride& o) {
        return o.command_id == command_id;
    });
    notify_change();
}
...
bool ShortcutConfig::has_override(const std::string& command_id) const {
    for (const auto& o : overrides_) {
        if (o.command_id == command_id) return true;
    }
    return false;
}
...
static bool read_json_bool(const std::string& json, const std::string& key, bool def) {
    ...
    if (rest.substr(start, 4) == "true") return true;
    if (rest.substr(start, 5) == "false") return false;
    return def;
}
...
bool ShortcutConfig::deserialize(const std::string& json) {
    ...
    if (vpos != std::string::npos) {
        ...
        if (cpos != std::string::npos) {
            ...
            if (ver > 1) return false;  // Future version
        }
    }
...
std::string ShortcutConfig::default_path() {
    ...
    if (!home) return "keybindings.json";

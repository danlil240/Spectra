...
std::vector<AutomationHandlerEntry> make_capture_handlers()
{
    ...
    // ── capture_screenshot ───────────────────────────────────────────────
    entries.push_back({"capture_screenshot",
                       [](AutomationRequest& req, App& app, WindowUIContext* ui_ctx)
                       {
                           ...
                           if (fig)
                           {
                               fig->save_png(path);
                               req.response_json =
                                   json_ok(req.id, "{\"path\":\"" + json_escape(path) + "\"}");
                           }
                           else
                           {
    ...
    // ── capture_window ───────────────────────────────────────────────────
    entries.push_back({"capture_window",
                       [](AutomationRequest& req, App& app, WindowUIContext* /*ui_ctx*/)
                       {
                           std::string path = json_get_string(req.params_json, "path");
                           if (path.empty())
                           ...
                           {
                               req.response_json =
                                   json_error(req.id, "Framebuffer readback failed");
                               return;
                           }

...
static void scene_mixed_series(App& /*app*/, Figure& fig) {
    ...
    for (size_t i = 0; i < N / 5; ++i) {
        ...
        y_scatter[i] = std::sin(x_scatter[i]) * 2.0f + 0.3f * std::cos(static_cast<float>(i) * 7.0f);
    }
...
static void scene_subplot_2x2(App& /*app*/, Figure& fig) {
    ...
    // Top-left: sin
    {
        ...
        ax.xlim(0.0f, 10.0f); ax.ylim(-1.5f, 1.5f);
        ax.title("sin(x)"); ax.grid(true);
    }
    // Top-right: cos
    {
    ...
    // Bottom-left: tan (clamped)
    {
        ...
        ax.xlim(0.0f, 10.0f); ax.ylim(-5.0f, 5.0f);
        ax.title("tan(x)"); ax.grid(true);
    }
    // Bottom-right: exp decay
    {
        auto& ax = fig.subplot(2, 2, 4);
        std::vector<float> y(N);
        for (size_t i = 0; i < N; ++i) y[i] = std::exp(-x[i] * 0.3f) * std::sin(x[i] * 3.0f);
        ax.line(x, y).label("decay").color(rgb(0.8f, 0.2f, 0.8f));
        ax.xlim(0.0f, 10.0f); ax.ylim(-1.5f, 1.5f);
        ax.title("Damped oscillation"); ax.grid(true);
    }
}
...
static void scene_subplot_3x1(App& /*app*/, Figure& fig) {
...

    {
        ...
        ax.xlim(0.0f, 10.0f); ax.ylim(-1.5f, 1.5f);
        ax.title("Channel 1"); ax.grid(true);
    }
    {
        auto& ax = fig.subplot(3, 1, 2);
        std::vector<float> y(N);
        for (size_t i = 0; i < N; ++i) y[i] = std::sin(x[i] * 2.0f) * 0.7f;
        ax.line(x, y).label("ch2").color(rgb(1.0f, 0.4f, 0.2f));
        ax.xlim(0.0f, 10.0f); ax.ylim(-1.5f, 1.5f);
        ax.title("Channel 2"); ax.grid(true);
    }
    {
        auto& ax = fig.subplot(3, 1, 3);
        std::vector<float> y(N);
        for (size_t i = 0; i < N; ++i) y[i] = std::sin(x[i] * 0.5f) * 1.2f;
        ax.line(x, y).label("ch3").color(rgb(0.3f, 0.9f, 0.3f));
        ax.xlim(0.0f, 10.0f); ax.ylim(-1.5f, 1.5f);
        ax.title("Channel 3"); ax.grid(true);
    }
}

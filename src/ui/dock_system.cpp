...
SplitPane* DockSystem::split_right(size_t new_figure_index, float ratio) {
    ...
    if (result && on_layout_changed_) on_layout_changed_();
    return result;
}
...
SplitPane* DockSystem::split_figure_right(size_t figure_index,
                                           size_t new_figure_index,
                                           float ratio) {
    auto* result = split_view_.split_pane(figure_index, SplitDirection::Horizontal,
                                           new_figure_index, ratio);
    if (result && on_layout_changed_) on_layout_changed_();
    return result;
}

SplitPane* DockSystem::split_figure_down(size_t figure_index,
                                          size_t new_figure_index,
                                          float ratio) {
    auto* result = split_view_.split_pane(figure_index, SplitDirection::Vertical,
                                           new_figure_index, ratio);
    if (result && on_layout_changed_) on_layout_changed_();
    return result;
}
...
DropTarget DockSystem::update_drag(float mouse_x, float mouse_y) {
    if (!is_dragging_) return DropTarget{};
...
bool DockSystem::end_drag(float mouse_x, float mouse_y) {
    ...
    if (target_figure == dragging_figure_index_ &&
        target.target_pane->figure_count() <= 1) {
        return false;
    }
    ...
    if (result && target.zone != DropZone::Center) {
        ...
        auto* dest_pane = needs_swap
            ? (parent ? parent->first() : result)
            : result;
...
bool DockSystem::move_figure_to_pane(size_t figure_index, SplitPane::PaneId target_pane_id) {
    ...
    if (!target || !target->is_leaf()) return false;

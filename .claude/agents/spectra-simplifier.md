...
Spectra ships an in-process HTTP MCP server that agents can use to control a live running instance. **Always kill existing instances before starting a new one, then start Spectra and connect through the MCP server.**

### Start/restart procedure

```bash
# Kill all running Spectra instances first
pkill -f spectra || true
sleep 0.5

# Launch Spectra (MCP server starts automatically on port 8765)
./build/app/spectra &

# Override port if needed
SPECTRA_MCP_PORT=9000 ./build/app/spectra &
```
...
`GET http://127.0.0.1:8765/` returns `{"name":"spectra-automation","status":"ok","endpoint":"..."}` — use this to verify the server is up before issuing commands.

...
float TimelineEditor::snap_time(float time) const {
    ...
    switch (snap_mode_) {
        case SnapMode::Frame: {
            if (fps_ <= 0.0f) return time;
            float frame_dur = 1.0f / fps_;
            return std::round(time / frame_dur) * frame_dur;
        }
        case SnapMode::Beat: {
            if (snap_interval_ <= 0.0f) return time;
...
void TimelineEditor::remove_track(uint32_t track_id) {
    std::lock_guard lock(mutex_);
    std::erase_if(tracks_, [track_id](const TimelineTrack& t) {
    ...
    for (auto& t : tracks_) {
        if (t.id == track_id) {
            ...
            for (const auto& kf : t.keyframes) {
                if (std::abs(kf.time - time) < 0.001f) return;
            }
...
void TimelineEditor::remove_keyframe(uint32_t track_id, float time) {
    ...
    for (auto& t : tracks_) {
        if (t.id == track_id) {
            ...
            auto it = std::find_if(t.keyframes.begin(), t.keyframes.end(),
...
void TimelineEditor::delete_selected() {
    ...
    for (auto& t : tracks_) {
        ...
        std::erase_if(t.keyframes, [](const KeyframeMarker& kf) {
            return kf.selected;
        });
    }
    fire_selection_change();
...
void TimelineEditor::fire_selection_change() {
    ...
    if (on_selection_change_) {
        ...
        for (auto& t : tracks_) {
            for (auto& kf : t.keyframes) {
                if (kf.selected) sel.push_back(&kf);
            }
        }
...
void TimelineEditor::draw(float width, float height) {
    ...
    if (time_range <= 0.0f) time_range = 1.0f;
    ...
    ImGui::SameLine();
    if (ImGui::SmallButton("|<")) {
        if (fps_ > 0.0f) playhead_ = std::max(0.0f, playhead_ - 1.0f / fps_);
    }
    ImGui::SameLine();
    if (ImGui::SmallButton(">|")) {
        if (fps_ > 0.0f) playhead_ = std::min(duration_, playhead_ + 1.0f / fps_);
    }
    ImGui::SameLine();
    ImGui::Text("%.2fs  F:%u/%u", playhead_,
    ...
    draw_list->AddRectFilled(
        ImVec2(origin.x + track_label_width, ruler_y),
        ImVec2(origin.x + width, ruler_y + ruler_height),
        IM_COL32(40, 40, 40, 255));

    // Tick marks
    float tick_spacing = 1.0f;  // 1 second
    if (px_per_sec < 30.0f) tick_spacing = 5.0f;
    else if (px_per_sec < 60.0f) tick_spacing = 2.0f;
    else if (px_per_sec > 300.0f) tick_spacing = 0.5f;
    else if (px_per_sec > 600.0f) tick_spacing = 0.1f;
    ...
    while (t <= view_end_) {
        ...
        draw_list->AddLine(
            ImVec2(px, ruler_y + ruler_height - tick_h),
    ...
    for (size_t i = 0; i < tracks_.size(); ++i) {
        ...
        // Track label
        ImU32 label_col = track.visible ? IM_COL32(200, 200, 200, 255) : IM_COL32(100, 100, 100, 128);
        ...
        if (track.locked) {
            draw_list->AddText(ImVec2(origin.x + track_label_width - 20, y + 6),
                               IM_COL32(200, 100, 100, 200), "L");
        }

        // Keyframe diamonds
        ImU32 kf_color = IM_COL32(
            static_cast<int>(track.color.r * 255),

...
static std::string serialize_node(const DockNode* node) {
    ...
    if (node->is_leaf()) {
        ...
        s += ",\"dir\":\"" + std::string(node->get_split_direction() == DockDirection::Horizontal ? "h" : "v") + "\"";
...
static std::unique_ptr<DockNode> deserialize_node(const std::string& data) {
    if (data.empty() || data == "null" || data[0] != '{') return nullptr;
    ...
    if (is_leaf) {
        ...
        if (!ct_str.empty()) ct = std::stoi(ct_str);
    ...
    if (!ratio_str.empty()) ratio = std::stof(ratio_str);
...
bool DockManager::deserialize_state(const std::string& state) {
    if (state.empty() || state[0] != '{') return false;
    ...
    auto find_val = [&](const std::string& key) -> std::string {
        ...
        if (pos == std::string::npos) return "";
        pos += search.size();
        while (pos < state.size() && state[pos] == ' ') ++pos;
        if (pos >= state.size()) return "";
        if (state[pos] == '"') {
            ...
            for (size_t i = pos; i < state.size(); ++i) {
                if (state[i] == '{') ++depth;
                    ...
                    --depth;
                    if (depth == 0) return state.substr(start, i - start + 1);
                }
            }
            return "";
        } else {
    ...
    if (!w_str.empty()) window_width_ = std::stof(w_str);
    if (!h_str.empty()) window_height_ = std::stof(h_str);

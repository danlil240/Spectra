...
int plotix_register_command(PlotixCommandRegistry registry,
    ...
    if (!registry || !desc || !desc->id || !desc->label) return -1;
...
int plotix_unregister_command(PlotixCommandRegistry registry,
    ...
    if (!registry || !command_id) return -1;
    auto* reg = static_cast<CommandRegistry*>(registry);
...
int plotix_push_undo(PlotixUndoManager manager,
    ...
    if (!manager || !description) return -1;
...
bool PluginManager::load_plugin(const std::string& path) {
    ...
    for (const auto& p : plugins_) {
        if (p.path == path && p.loaded) return false;
    }
...
std::vector<std::string> PluginManager::discover(const std::string& directory) const {
    ...
    try {
        if (!std::filesystem::exists(directory)) return paths;
...
std::string PluginManager::serialize_state() const {
    ...
    for (size_t i = 0; i < plugins_.size(); ++i) {
        ...
        out += "    {\"name\": \"" + escape_json(p.name) +
               "\", \"path\": \"" + escape_json(p.path) +
               "\", \"enabled\": " + (p.enabled ? "true" : "false") + "}";
        if (i + 1 < plugins_.size()) out += ",";
        out += "\n";
    }
    out += "  ]\n}\n";
    return out;

...
static int string_to_key(const std::string& str) {
    ...
    if (lower == "pageup")    return KEY_PAGE_UP;
    if (lower == "pagedown")  return KEY_PAGE_DOWN;
...
std::string Shortcut::to_string() const {
    std::string result;
    if (has_mod(mods, KeyMod::Control)) result += "Ctrl+";
    if (has_mod(mods, KeyMod::Shift))   result += "Shift+";
    if (has_mod(mods, KeyMod::Alt))     result += "Alt+";
    if (has_mod(mods, KeyMod::Super))   result += "Super+";
...
Shortcut Shortcut::from_string(const std::string& str) {
    ...
    for (size_t i = 0; i + 1 < parts.size(); ++i) {
        ...
        if (lower == "ctrl" || lower == "control") s.mods = s.mods | KeyMod::Control;
        else if (lower == "shift") s.mods = s.mods | KeyMod::Shift;
        else if (lower == "alt")   s.mods = s.mods | KeyMod::Alt;
...
void ShortcutManager::unbind_command(const std::string& command_id) {
    std::lock_guard lock(mutex_);
    std::erase_if(bindings_, [&](const auto& pair) {
        return pair.second == command_id;
    });
}

...
TransitionEngine::animate(float& target, float end, float duration,
                          EasingFunc easing) {
...
TransitionEngine::AnimId
TransitionEngine::animate(Color& target, Color end, float duration,
                          EasingFunc easing) {
    std::lock_guard lock(mutex_);
...
TransitionEngine::AnimId
TransitionEngine::animate_limits(Axes& axes,
...
TransitionEngine::AnimId
TransitionEngine::animate_inertial_pan(Axes& axes,
...
void TransitionEngine::cancel(AnimId id) {
    std::lock_guard lock(mutex_);
    for (auto& a : float_anims_)    if (a.id == id) a.finished = true;
...
bool TransitionEngine::has_active_animations() const {
    std::lock_guard lock(mutex_);
    for (const auto& a : float_anims_)    if (!a.finished) return true;
...
size_t TransitionEngine::active_count() const {
    std::lock_guard lock(mutex_);
    size_t count = 0;
    for (const auto& a : float_anims_)    if (!a.finished) ++count;

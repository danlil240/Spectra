...
bool CommandPalette::draw(float window_width, float window_height) {
    ...
    float total_content_h = 0.0f;
    {
        ...
        for (int i = 0; i < static_cast<int>(results_.size()); ++i) {
            if (!results_[i].command) continue;
    ...
    // ─── Scrollbar drag handling (before scroll physics) ────────────────────
    {
        ...
        bool mouse_in_sb = scrollable && sb_thumb_h > 0.0f &&
            mouse.x >= (sb_left - SB_HIT_PAD) && mouse.x <= (sb_right + SB_HIT_PAD) &&
            mouse.y >= sb_track_top && mouse.y <= sb_track_bot;

        if (scrollbar_dragging_) {
            if (ImGui::IsMouseDown(0)) {
                float new_thumb_top = mouse.y - scrollbar_drag_offset_;
                float clamped = std::max(sb_track_top,
                    std::min(new_thumb_top, sb_track_bot - sb_thumb_h));
                float ratio = (sb_track_h > sb_thumb_h)
                    ? (clamped - sb_track_top) / (sb_track_h - sb_thumb_h) : 0.0f;
    ...
    // ─── Scrollbar opacity animation ────────────────────────────────────────
    {
        ...
        if (scrollbar_opacity_ < 0.01f) scrollbar_opacity_ = 0.0f;
    }
    ...
    fg->AddRectFilled(
        ImVec2(0, 0), ImVec2(window_width, window_height),
        IM_COL32(
            static_cast<int>(colors.bg_overlay.r * 255),
    ...
    if (ImGui::IsMouseClicked(0)) {
        ...
        bool on_palette = mp.x >= palette_x && mp.x <= palette_x + palette_w &&
    ...
    if (ImGui::Begin("##command_palette", nullptr, palette_flags)) {
        ...
        // ─── Results list ───────────────────────────────────────────────
        if (!results_.empty()) {
            ImGui::BeginChild("##palette_results", ImVec2(0, visible_height_), false,
    ...
    if (scrollable && sb_thumb_h > 0.0f && scrollbar_opacity_ > 0.01f) {
        ...
        if (scrollbar_hover_t_ > 0.05f) {
            ...
            fg->AddRectFilled(
                ImVec2(final_sb_left, sb_track_top),
        ...
        // Thumb — pill-shaped
        int thumb_alpha = static_cast<int>(
            (0.35f + 0.35f * scrollbar_hover_t_) * alpha);
        if (scrollbar_dragging_) thumb_alpha = static_cast<int>(0.8f * alpha);

        fg->AddRectFilled(
            ImVec2(final_sb_left, final_thumb_top),

...
struct ParticleSystem {
    ...
    void update(float dt, float time) {
        for (size_t i = 0; i < px.size(); ++i) {
            ...
            if (std::abs(px[i]) > 3.0f) px[i] = -px[i];
            if (std::abs(py[i]) > 3.0f) py[i] = -py[i];
            if (std::abs(pz[i]) > 3.0f) pz[i] = -pz[i];
...
int main() {
    ...
    fig.animate()
        ...
        .on_frame([&](Frame& frame) {
            ...
            ax.camera().elevation = 45.0f + 30.0f * std::sin(t * 2.0f * M_PI)
                                  + 15.0f * std::cos(t * 5.0f * M_PI);

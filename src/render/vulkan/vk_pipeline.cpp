...
VkPipeline create_graphics_pipeline(VkDevice device, const PipelineConfig& config) {
    VkShaderModule vert_module = create_shader_module(device, config.vert_spirv, config.vert_spirv_size);
    VkShaderModule frag_module = create_shader_module(device, config.frag_spirv, config.frag_spirv_size);

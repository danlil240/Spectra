...
void RegionSelect::data_to_screen(float data_x, float data_y,
                                   const Rect& viewport,
...
void RegionSelect::screen_to_data(double screen_x, double screen_y,
                                   const Rect& viewport,
                                   float xlim_min, float xlim_max,
                                   float ylim_min, float ylim_max,
...
void RegionSelect::begin(double screen_x, double screen_y,
                          const Rect& viewport,
                          float xlim_min, float xlim_max,
                          float ylim_min, float ylim_max) {
    dragging_ = true;
    ...
    screen_to_data(screen_x, screen_y, viewport,
...
void RegionSelect::update_drag(double screen_x, double screen_y,
                                const Rect& viewport,
                                float xlim_min, float xlim_max,
                                float ylim_min, float ylim_max) {
    if (!dragging_) return;
...
void RegionSelect::collect_points(const Axes* axes) {
    ...
    for (auto& series_ptr : axes->series()) {
        ...
        for (size_t i = 0; i < count; ++i) {
            if (x_data[i] >= xmin && x_data[i] <= xmax &&
                y_data[i] >= ymin && y_data[i] <= ymax) {
...
void RegionSelect::compute_statistics() {
    stats_ = {};
    if (selected_points_.empty()) return;
    ...
    if (stats_.point_count > 1) {
        ...
        stats_.y_std = static_cast<float>(std::sqrt(sum_sq / static_cast<double>(stats_.point_count - 1)));
    }
}
...
void RegionSelect::draw(const Rect& viewport,
    ...
    data_to_screen(data_x0_, data_y0_, viewport,
                   xlim_min, xlim_max, ylim_min, ylim_max, sx0, sy0);
    data_to_screen(data_x1_, data_y1_, viewport,
                   xlim_min, xlim_max, ylim_min, ylim_max, sx1, sy1);
    ...
    // Selection fill
    ImU32 fill_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
        colors.selection_fill.r, colors.selection_fill.g,
        colors.selection_fill.b, fill_alpha_ * opacity_));
    fg->AddRectFilled(ImVec2(rx0, ry0), ImVec2(rx1, ry1), fill_col);

    // Selection border
    ImU32 border_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
        colors.selection_border.r, colors.selection_border.g,
        colors.selection_border.b, colors.selection_border.a * opacity_));
...
void RegionSelect::draw_mini_toolbar(float rx0, float ry0, float rx1, float ry1,
                                      float window_width, float window_height) {
    const auto& colors = ui::ThemeManager::instance().colors();
    ...
    char range_buf[96];
    std::snprintf(range_buf, sizeof(range_buf), "X: [%.4g, %.4g]  Y: [%.4g, %.4g]",
                  stats_.x_min, stats_.x_max, stats_.y_min, stats_.y_max);
    ...
    // Clamp to window
    if (tx < 4.0f) tx = 4.0f;
    if (tx + toolbar_w > window_width - 4.0f) tx = window_width - toolbar_w - 4.0f;
    if (ty + toolbar_h > window_height - 4.0f) ty = ry0 - toolbar_h - 8.0f;
    if (ty < 4.0f) ty = 4.0f;
    ...
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(
        colors.bg_elevated.r, colors.bg_elevated.g,
        colors.bg_elevated.b, 0.95f));
    ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(
        colors.border_default.r, colors.border_default.g,
        colors.border_default.b, colors.border_default.a));
    ...
    if (ImGui::Begin("##region_stats", nullptr, flags)) {
        ImU32 text_primary = ImGui::ColorConvertFloat4ToU32(ImVec4(
            colors.text_primary.r, colors.text_primary.g,
            colors.text_primary.b, colors.text_primary.a));
        ImU32 text_secondary = ImGui::ColorConvertFloat4ToU32(ImVec4(
            colors.text_secondary.r, colors.text_secondary.g,
            colors.text_secondary.b, colors.text_secondary.a));
        ImU32 accent_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
        ...
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(
            colors.accent_subtle.r, colors.accent_subtle.g,
            colors.accent_subtle.b, colors.accent_subtle.a));

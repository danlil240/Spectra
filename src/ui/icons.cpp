...
bool IconFont::initialize() {
    ...
    if (atlas) {
        for (ImFont* font : atlas->Fonts) {
            ...
            if (sz >= 15.5f && sz <= 16.5f) font_16_ = font;
            else if (sz >= 19.5f && sz <= 20.5f) font_20_ = font;
            else if (sz >= 17.5f && sz <= 18.5f) font_24_ = font;  // use 18px as closest to 24
        }
...
ImFont* IconFont::get_font(float size) const {
    ...
    if (size <= 16.0f) return font_16_;
    if (size <= 20.0f) return font_20_;
    if (size <= 24.0f) return font_24_;
    return font_32_;
}

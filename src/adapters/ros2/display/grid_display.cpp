...
std::string GridDisplay::serialize_config_blob() const
{
    ...
    std::snprintf(buffer,
                  sizeof(buffer),
                  "cell_size=%.3f;cell_count=%d;plane=%s;color=%.3f,%.3f,%.3f;alpha=%.3f;offset=%.3f,%.3f,%.3f",
                  cell_size_,
                  cell_count_,
                  plane_.c_str(),
...
void GridDisplay::submit_renderables(SceneManager& scene)
{
    ...
    std::snprintf(color_buffer, sizeof(color_buffer),
                  "%.3f, %.3f, %.3f, %.3f",
                  color_[0], color_[1], color_[2], alpha_);

...
static void collect_numeric_paths_recursive(const std::vector<FieldDescriptor>& fields,
...
static const FieldDescriptor* find_field_recursive(const std::vector<FieldDescriptor>& fields,
...
double FieldAccessor::extract_double(const void* msg_ptr, size_t array_index) const
{
    ...
    for (const auto& step : steps_)
    {
        ...
        if (step.is_dynamic)
        {
            const void* const* data_ptr = reinterpret_cast<const void* const*>(parent);
...
void MessageIntrospector::build_fields(const rosidl_message_type_support_t* ts,
...
//   → sym:
//   rosidl_typesupport_introspection_cpp__get_message_type_support_handle__std_msgs__msg__Float64
static bool parse_type_string(const std::string& type_name,
                              std::string&       package,
...
[[maybe_unused]] static std::string to_symbol_part(const std::string& s)
{
    std::string r = s;
    for (char& c : r)
        if (c == '/')
...
std::shared_ptr<const MessageSchema> MessageIntrospector::introspect(const std::string& type_name)
{
    {
    ...
    const std::string lib_name = "lib" + package + "__rosidl_typesupport_introspection_cpp.so";
...
std::shared_ptr<const MessageSchema> MessageIntrospector::introspect_type_support(
    const rosidl_message_type_support_t* ts,
    const std::string&                   type_name)
{
    if (!ts)
...
bool MessageIntrospector::build_accessor_steps(const std::vector<FieldDescriptor>& fields,
...
FieldAccessor MessageIntrospector::make_accessor(const MessageSchema& schema,

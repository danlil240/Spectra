...
static void BM_AnimationController_Update_50Anims(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        ctrl.cancel_all();
        for (int i = 0; i < 50; ++i) {
            axes[i].xlim(0, 10); axes[i].ylim(0, 10);
...
NearestResult find_nearest_bench(
    float cursor_x, float cursor_y,
    const float* x_data, const float* y_data, size_t count,
    const Rect& viewport, float xlim_min, float xlim_max,
...
static void BM_NearestPoint_1K(benchmark::State& state) {
    ...
    for (auto _ : state) {
        auto r = find_nearest_bench(960.0f, 540.0f, x.data(), y.data(), N,
                                     vp, 0, 100, 0, 100);

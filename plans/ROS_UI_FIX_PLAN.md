...
## 5. UI Polish Status (June 2026)

Living checklist for the UI Polish quarter plan. Update as acceptance criteria are verified.

| Phase | Criterion | Status | Notes |
|-------|-----------|--------|-------|
| P1 | Single menu / no Spectra chrome bleed | Done | `main.cpp` suppresses command bar, canvas, status bar |
| P1 | Docking works | Done | ImGui dockspace + reset layout |
| P2 | Context flows (topic → echo + stats) | Done | `RosWorkspaceState` + `on_topic_selected` |
| P2 | Echo pin when manually overridden | Done | `TopicEchoPanel::manually_pinned_` |
| P3 | Layout persistence | Done | `imgui_layout` in `RosSession` |
| P3 | Default dock spec | Done | `apply_default_dock_layout()` ratios tuned |
| P4 | Field tree + drag-drop + double-click plot | Done | Topic Monitor inline echo + drop targets |
| P4 | Per-series controls in stats | Done | `TopicStatsOverlay::draw_series_controls` |
| P4 | Plot in &lt;10 s workflow | Verify | Manual timed test with `/cmd_vel` |
| 3D | Viewport toolbar (views, fixed frame) | Done | Scene viewport + menu bar fixed frame |
| 3D | Inspector empty state | Done | Fixed frame + entity count when unselected |
| QA | Design-review manifest expanded | Done | Extra captures in `scenario_design_review` |
| Stretch | OccupancyGrid display | Done | `OccupancyGridDisplay` plugin |
| Stretch | Measure tool | Done | `MeasureTool` in scene viewport |
| Stretch | Package URI resolver | Done | `ros_package_resolver` for `package://` |

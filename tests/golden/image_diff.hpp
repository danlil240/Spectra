...
inline DiffResult compare_images(const uint8_t* actual, const uint8_t* expected,
                                  uint32_t width, uint32_t height,
                                  uint8_t threshold = 2) {
    DiffResult result;
    result.total_pixels = static_cast<size_t>(width) * height;

    if (result.total_pixels == 0) return result;
    ...
    for (size_t i = 0; i < result.total_pixels; ++i) {
        ...
        for (int c = 0; c < 4; ++c) {
            ...
            if (diff > threshold) pixel_differs = true;
        }
    ...
    result.percent_different   = (static_cast<double>(result.differing_pixels) /
                                  static_cast<double>(result.total_pixels)) * 100.0;
    return result;
}

// Write a simple diff visualization: red pixels where images differ.
// Returns RGBA buffer of same dimensions.
inline std::vector<uint8_t> generate_diff_image(const uint8_t* actual, const uint8_t* expected,
                                                  uint32_t width, uint32_t height,
                                                  uint8_t threshold = 2) {
    ...
    for (size_t i = 0; i < total; ++i) {
        ...
        for (int c = 0; c < 4; ++c) {
            int d = std::abs(static_cast<int>(actual[base + c]) -
                             static_cast<int>(expected[base + c]));
            if (d > threshold) pixel_differs = true;
        }

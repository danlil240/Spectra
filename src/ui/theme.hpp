...
struct Color {
    ...
    HSL to_hsl() const {
        ...
        if (max_c == min_c) return {0.0f, 0.0f, l};
        float d = max_c - min_c;
    ...
    static Color from_hsl(float h, float s, float l, float a = 1.0f) {
        if (s == 0.0f) return Color(l, l, l, a);
...
struct DataPalette {
    ...
    bool is_safe_for(CVDType type) const {
        if (type == CVDType::None) return true;
        for (auto t : safe_for) if (t == type) return true;
        return false;
    }
};
...
public:
    ...
    void transition_palette(const std::string& palette_name, float duration_sec = tokens::DURATION_SLOW);
...
inline const DataPalette& data_palette() { return ThemeManager::instance().current_data_palette(); }

...
> Last updated: 2026-03-06 (Performance Agent Session 12) | Session seeds: 42, 12345, 99999, 77777, 1771883518, 1771883726, 1771883913, 1771884136, 1771959053, 42 (perf), 99 (perf), 42 (session 25), 42 (perf-agent), 23756320363876 (perf-agent random), 13062186744256 (perf-agent repro), 18335134330653 (memory-agent), 42 (perf-agent repro #2), 42 (perf-agent verify #2), 27243840318184 (perf-agent random #2), 42 (memory-agent telemetry), 42 (real desktop perf), 35619058242308 (real desktop perf random), 42 (perf-agent session 12), 25257321839906 (perf-agent random #3), 99 (nvidia driver crash), 26349518760598 (perf-agent regression)
...
### C7: X11 BadLength Crash From Oversized Clipboard Write
- **Observed (Performance Agent session, 2026-03-06, seed 42):** X11 `BadLength` error at frame 3059 during `command_exhaustion` scenario. `glfwSetClipboardString` passed a multi-megabyte TSV string generated by `data.copy_all` after `massive_datasets` created millions of data points; X11 protocol rejects payloads >4 MB.
- **Root cause:** `data.copy_all` command serializes all visible series data to clipboard as TSV. After `massive_datasets`, series hold millions of points; the resulting TSV string exceeds the X11 maximum request size, causing a fatal protocol error.
- **Fix applied:** Added a `kMaxClipboardBytes = 4 * 1024 * 1024` guard in `register_commands.cpp` before calling `glfwSetClipboardString`. If the TSV exceeds 4 MB, a `SPECTRA_LOG_WARN` is emitted instead of attempting the clipboard write.
- **Files modified:**
  - `src/ui/app/register_commands.cpp` — clipboard size guard before `glfwSetClipboardString`
- **Verified:** seed 42 no longer crashes at frame 3059 from clipboard overflow.
- **Priority:** **P0**
- **Status:** ✅ Fixed

### C8: OOM (std::bad_alloc) From Unbounded series.paste During Fuzzing
- **Observed (Performance Agent session, 2026-03-06, seed 42):** `std::bad_alloc` at frame 3059 after repeated `series.paste` fuzz actions accumulated hundreds of pasted series per axes without limit, exhausting available memory.
- **Root cause:** The `series.paste` command had no upper bound on how many series could be pasted into a single axes. During extended fuzzing, the accumulation of copied series consumed all available memory.
- **Fix applied:** Added `kMaxSeriesPerAxes = 200` guard in `register_commands.cpp`. If the target axes already has 200+ series, the paste is skipped with a `SPECTRA_LOG_WARN`.
- **Files modified:**
  - `src/ui/app/register_commands.cpp` — series count guard in `series.paste` command
- **Verified:** seed 42 completes full session without OOM.
- **Priority:** **P0**
- **Status:** ✅ Fixed

### C9: SIGSEGV in file.save_workspace — Dangling Series* in DataInteraction Markers
- **Observed (Performance Agent session, 2026-03-06, seed 42):** SIGABRT/SIGSEGV at frame 3059 during `file.save_workspace` command. Crash in `m.series->label()` where `m.series` was a dangling `Series*` pointer in `DataInteraction::markers_`.
- **Root cause:** `DataInteraction::markers_` holds `DataMarker` structs containing raw `Series*` pointers. When figures are destroyed (e.g., via `fuzz:CloseFigure`), `clear_figure_cache()` nulled `last_figure_` and `active_axes_` but did not clear the `markers_` vector. The `file.save_workspace` command iterated over `markers_` and dereferenced `m.series->label()` on dangling pointers.
- **Fix applied:**
  1. Added `markers_.clear()` to `DataInteraction::clear_figure_cache()` to prevent dangling `Series*` pointers in markers when a figure is destroyed.
  2. Changed `file.save_workspace` marker serialization to use the pre-stored `m.series_label` string instead of dereferencing `m.series->label()`.
- **Files modified:**
  - `src/ui/overlay/data_interaction.hpp` — added `markers_.clear()` in `clear_figure_cache()`
  - `src/ui/app/register_commands.cpp` — use `m.series_label` instead of `m.series->label()` in save command
- **Verified:** seed 42 completes 6314 frames, 20/20 scenarios, no crash. Random seeds also clean.
- **Priority:** **P0**
- **Status:** ✅ Fixed

### Known: Nvidia Driver SIGSEGV During WindowResize Swapchain Destruction
- **Observed (Performance Agent session, 2026-03-06, seed 99):** SIGSEGV at frame 4960 during `fuzz:WindowResize` scenario. Crash inside `libnvidia-glcore.so.580.126.20` during `vkDestroyImageView` in swapchain recreation.
- **Root cause:** Nvidia driver-level bug — the crash occurs inside the proprietary driver code, not in Spectra application logic. Attempted `vkDeviceWaitIdle` before swapchain recreation but the crash persists.
- **Impact:** Only observed with Nvidia driver 580.126.20 under aggressive WindowResize fuzzing. Not reproducible on every seed.
- **Priority:** N/A (driver bug, not fixable at application level)
- **Status:** 📝 Documented — driver-level issue
...
- **Status:** � Significantly improved
  - Full 20-scenario session (2026-03-17): end RSS +78MB (198→276MB), peak 408MB.
  - `command_exhaustion` down from +115MB to +11MB after cleanup (undo stack clear + `reset_to_single_window_lightweight_state`).
  - `massive_datasets` +38MB confirmed as glibc allocator retention (3 independent runs all show identical +39MB plateau, not linear growth). Not a Spectra leak.
  - GPU device-local flat at +0MB across all 20 scenarios.
  - ASan full session (20 scenarios): 0 memory safety errors.
  - Remaining RSS growth is allocator retention from swapchain recreation and large transient allocations.

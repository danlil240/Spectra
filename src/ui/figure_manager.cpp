...
FigureManager::FigureManager(std::vector<std::unique_ptr<Figure>>& figures)
    : figures_(figures)
{
    ensure_states();
}
...
void FigureManager::sync_tab_bar() {
    ...
    while (tab_bar_->get_tab_count() > 1) {
        ...
        if (!removed) break;  // Only non-closeable tabs remain
    }

...
LegendSeriesState& LegendInteraction::get_state(const Series* s) {
    ...
    if (it != series_states_.end()) return it->second;
        ...
        for (auto it = series_states_.begin(); it != series_states_.end(); ) {
            ...
            for (auto& axes_ptr : figure.axes()) {
                ...
                for (auto& s : axes_ptr->series()) {
                    if (s.get() == it->first) { found = true; break; }
                }
                if (found) break;
            }
            if (!found) {
...
bool LegendInteraction::draw(Axes& axes, const Rect& viewport, size_t axes_index) {
    ...
    if (series_list.empty()) return false;

    // Count labeled series
    int labeled_count = 0;
    for (auto& s : series_list) {
        if (s && !s->label().empty()) ++labeled_count;
    }
    if (labeled_count == 0) return false;
    ...
    if (toggleable_) legend_w += eye_width + 4.0f;
    ...
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(
        ...
        colors.bg_elevated.b, 0.92f));
    ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(
        colors.border_subtle.r, colors.border_subtle.g,
        colors.border_subtle.b, colors.border_subtle.a));
    ...
    if (ImGui::Begin(win_id, nullptr, flags)) {
        ...
        for (auto& s : series_list) {
            ...
            // Color swatch
            ImU32 swatch_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
                sc.r, sc.g, sc.b, sc.a * vis_alpha));
            float swatch_y = row_y + (row_height - swatch_size) * 0.5f;
            dl->AddRectFilled(
                ImVec2(row_x, swatch_y),
            ...
            ImU32 text_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
                ...
                colors.text_primary.b, vis_alpha));
            float label_x = row_x + swatch_size + swatch_gap;
            float label_y = row_y + (row_height - font_size) * 0.5f;
            dl->AddText(font, font_size, ImVec2(label_x, label_y),
                        text_col, s->label().c_str());
            ...
            if (toggleable_) {
                ...
                if (ImGui::IsItemHovered()) {
                    ImU32 hover_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
                        colors.accent_subtle.r, colors.accent_subtle.g,
                        colors.accent_subtle.b, 0.3f));
                    dl->AddRectFilled(
                ...
                ImU32 eye_col = ImGui::ColorConvertFloat4ToU32(ImVec4(
                    colors.text_tertiary.r, colors.text_tertiary.g,
                    colors.text_tertiary.b, colors.text_tertiary.a));
                dl->AddText(font, font_size * 0.7f, ImVec2(eye_x, eye_y),
                            eye_col, eye_label);
            }

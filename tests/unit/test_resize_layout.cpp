...
TEST(ResizeLayout, MultiSubplotStaysInBoundsAcrossResizes) {
    ...
    float sizes[][2] = {
        {1920.0f, 1080.0f}, {1600.0f, 900.0f}, {1024.0f, 768.0f},
        {640.0f, 480.0f}, {320.0f, 240.0f}, {1920.0f, 1080.0f}
    };
    ...
    for (auto& sz : sizes) {
        ...
        for (size_t i = 0; i < rects.size(); ++i) {
            EXPECT_GE(rects[i].w, 0.0f)
                << "axes " << i << " at " << sz[0] << "x" << sz[1];
            EXPECT_GE(rects[i].h, 0.0f)
                << "axes " << i << " at " << sz[0] << "x" << sz[1];
